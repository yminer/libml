...
(**
  The learnMlpNNWeighDecayVisitor class
...
class learnMlpnnWeightDecayVisitor =
object
  inherit [mlpNN] learnVisitor

  initializer
    _moduleName <- "MLPNN weight decay learning"

    (**
      Weight decay coefficient.
    *)
  val mutable d = 0.000001
	...
	for l = network#getLayerNb - 2 downto 0 do
	  for i = 0 to (Array.length !weights.(l)) - 1 do
	    for j = 0 to (Array.length !weights.(l).(i)) - 1 do
	      !weights.(l).(i).(j)
	      <- !weights.(l).(i).(j)
	      -. network#getStep *. !gradients.(l).(i).(j)
	      -. d *. !weights.(l).(i).(j)
	    done
	  done
	done
