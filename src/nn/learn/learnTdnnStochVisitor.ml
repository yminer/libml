...
(**
  The learnTdNNStochVisitor class
...
class learnTdnnStochVisitor =
object
  inherit [tdNN] learnVisitor
    
  initializer
    _moduleName <- "TDNN stochastic learning"
	...
	for l = network#getLayerNb - 2 downto 0 do
	  for i = 0 to (Array.length !weights.(l)) - 1 do
	    for j = 0 to (Array.length !weights.(l).(i)) - 1 do
	      for k = 0 to (Array.length !weights.(l).(i).(j)) - 1 do
		!weights.(l).(i).(j).(k)
		<- !weights.(l).(i).(j).(k) +. network#getStep
		    *. !gradients.(l).(i).(j).(k)
	      done
	    done
	  done
