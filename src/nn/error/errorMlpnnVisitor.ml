...
object
  ...
  *)
  method visit (network : mlpNN) =
	...
	*)
	for i = 0 to neuronsPerLayer.(layerNb - 1) do
	  !error.(layerNb - 1).(i)
	  <-  derivate(!inputSum.(layerNb - 1).(i)) 
	  *. (!outputActivation.(layerNb - 1).(i) -. output.(i));
	  network#addLearningError ((!outputActivation.(layerNb - 1).(i) -. output.(i))
				    *. (!outputActivation.(layerNb - 1).(i) -. output.(i)))
	done;
	(**
	...
	*)
	for l = layerNb - 2 downto 0 do
	  for i = 0 to neuronsPerLayer.(l) - 1 do
	    !error.(l).(i) <- 0.;
	    for j = 0 to neuronsPerLayer.(l + 1) - 1 do
	      !error.(l).(i)
	      <- !error.(l).(i)
