...
(**
  The errorMlpnnOptimizedVisitor class
  Compute the error and retropropagate it trought the network.
  This version using a static version fo the transfert functions.
...
*)
class errorMlpnnOptimizedVisitor =
object
	...
	*)
	for l = nbL - 2 downto 0 do
	  for i = 0 to neurons.(l) - 1 do
	    ...
	    !error.(l).(i) <- !error.(l).(i) *. expDerivate(!inputSum.(l).(i))
	  done
	done
      end
